...
async function redrive() {
  while (true) {
    const fromMessage = await sqsClient.receiveMessage({
      QueueUrl: fromQueue,
      AttributeNames: ['All'],
    ...
    const receipt = await sqsClient.sendMessage({
      QueueUrl: toQueue,
    ...
    if (receipt.MessageId) {
      ...
      console.log(`Message with body: ${srcMessage.Body} and MessageGroupId ${srcMessage.Attributes!.MessageGroupId} reprocessed`);
    } else {
      throw new Error(`No message receipt for source message: ${JSON.stringify(srcMessage)}`);
    }
  }
}
