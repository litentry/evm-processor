...
export default function getHandler(client: Client) {
  ...
  return async function handler(startBlock: number, endBlock: number) {
    ...
    async function contractCreationTransactions() {
      ...
      const querySql = transaactions.map(
      ...
      const sql =
        `INSERT INTO evm_contract_creation_transactions (
          id,
          hash,
          nonce,
          blockHash,
    ...
    async function contractTransactions() {
      ...
      const querySql = transactions.map(
      ...
      const sql =
        `INSERT INTO evm_contract_transactions (
          id,
          hash,
          nonce,
          blockHash,
    ...
    async function nativeTokenTransactions() {
      ...
      const sql =
        `INSERT INTO evm_native_token_transactions (
          id,
          hash,
          nonce,
          blockHash,
    ...
    async function erc20Contracts() {
      ...
      const sql =
        `INSERT INTO evm_contracts_erc20 (
          address,
          creator,
          blockNumber,
          timestamp,
    ...
    async function erc721Contracts() {
      ...
      const sql =
        `INSERT INTO evm_contracts_erc721 (
          address,
          creator,
          blockNumber,
          timestamp,
    ...
    async function erc1155Contracts() {
      ...
      const sql =
        `INSERT INTO evm_contracts_erc1155 (
          address,
          creator,
          blockNumber,
          timestamp,
    ...
    async function allTokenActivityEvents() {
      for (const ercType of ercTypes) {
        ...
        const sql =
          `INSERT INTO evm_token_activity_event_erc${ercType} (
            contract,
            transactionHash,
            signature,
            blockNumber,
    ...
    async function allTokenActivityTransactions() {
      for (const ercType of ercTypes) {
        const transactions: DecodedContractTransaction[] = await query.tokenActivity.transactions({
          blockRange: {
            start: startBlock,
            end: endBlock,
          },
        ...
        const sql =
          `INSERT INTO evm_token_activity_transaction_erc${ercType} (
            hash,
            contract,
            signer,
            signature,
